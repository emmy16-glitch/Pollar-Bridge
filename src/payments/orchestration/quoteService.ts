...
import { assertFinitePositive, assertNonBlank } from "../../validation.js";
...
export class QuoteService {
  createQuote(req: QuoteRequest): Quote {
    const corridorId = assertNonBlank(req?.corridorId, "corridorId");
    // NaN/Infinity slip past range checks (all comparisons false for NaN),
    // so reject non-finite amounts explicitly — never mint a NaN quote.
    const sourceAmount = assertFinitePositive(req?.sourceAmount, "sourceAmount");
    const corridor = getCorridor(corridorId);
    if (!corridor.enabled) throw new Error(`Corridor disabled: ${corridorId}`);
    if (sourceAmount < corridor.minimumAmount || sourceAmount > corridor.maximumAmount) {
    ...
    const providerFee = Math.max(1, Math.round(sourceAmount * 0.008));
    const platformFee = Math.max(1, Math.round(sourceAmount * 0.007));
    const totalFees = providerFee + platformFee;
    const net = sourceAmount - totalFees;
    ...
    return {
      ...
      totalRequired: sourceAmount,
      amountDue: sourceAmount,
